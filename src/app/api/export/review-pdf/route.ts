...
import { getResumeById, getLatestReview, getReviewByVersion } from "@/lib/db/queries";
...
export async function GET(request: NextRequest) {
  ...
  const reviewRecord = versionParam
    ? getReviewByVersion(resumeId, parseInt(versionParam, 10))
    : getLatestReview(resumeId);
  ...
  try {
    ...
    const filename = `revue-cv-${safeName}-v${reviewRecord.version}.pdf`;
